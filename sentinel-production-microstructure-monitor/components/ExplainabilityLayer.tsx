...
export const ExplainabilityLayer: React.FC<ExplainabilityLayerProps> = ({ trace, stress, signals }) => {
  ...
  // Verify the EMA formula from displayed values so the user can check it
  // smoothed = alpha × pre_smooth + (1 - alpha) × previous
  const verifiedSmoothed = trace.smoothing_alpha * trace.pre_smooth_score +
                           (1 - trace.smoothing_alpha) * trace.previous_score;
  const verifiedFinal    = Math.round(verifiedSmoothed);
  // If the displayed final score matches our re-derived value, the pipeline is consistent
  const pipelineVerified = verifiedFinal === trace.final_score;
  ...
  return (
    <div className="bg-[#0d1117] border border-gray-800 rounded-xl overflow-hidden flex flex-col shadow-2xl animate-in fade-in duration-500">
      ...
      <div className="px-6 py-3 border-b border-gray-800 flex justify-between items-center bg-[#151a23]">
        <div className="flex items-center gap-3">
          ...
          {/* RENAMED: "Reasoning Engine Audit" → "Score Computation Trace"
              There is no reasoning or AI here. This panel shows the exact
              arithmetic steps: how raw signal values flow through weights,
              shock multiplier, and EMA smoothing to produce the final score. */}
          <h2 className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.3em] font-mono">Score Computation Trace</h2>
        </div>
        <div className="flex items-center gap-4 font-mono text-[10px]">
          <span className="text-gray-600 uppercase">Previous → Final:</span>
          <span className="text-gray-400">
            {trace.previous_score.toFixed(1)} → <span className="text-white font-bold">{trace.final_score}</span>
          </span>
          {/* Live verification badge */}
          <span className={`text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-widest ${pipelineVerified ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'}`}>
            {pipelineVerified ? '✓ Verified' : '⚠ Mismatch'}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6 p-6">
          ...
          <div className="space-y-4">
            {[...trace.weight_contributions]
              ...
              .map((contrib, i) => {
                ...
                // Verify each contribution: weight × raw_value should equal contribution
                  ...
                  <div key={i} className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[9px] font-mono text-gray-500 italic">
                        ...
                        {/* Show percentage of total so bar label matches bar width */}
                        <span className="text-gray-600 ml-1">({derivedPct.toFixed(0)}%)</span>
                      </span>
                    </div>
        ...
        {/* FIX: added "Prev Score" box into the chain so the EMA step is fully
            verifiable from the displayed values alone.
            Old pipeline: Raw → Shock → Pre-Smooth → Alpha → Final
            New pipeline: Raw → Shock → Pre-Smooth → EMA(α, prev) → Final
            Now you can check: α×Pre-Smooth + (1-α)×Prev = smoothed = Final */}
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-2 scrollbar-none">
            ...
            {/* Box 1: Raw */}
            <div className="flex-1 min-w-[80px] bg-gray-900/50 border border-gray-800 rounded p-2.5 flex flex-col items-center">
            ...
            {/* Box 2: Shock */}
            <div className={`flex-1 min-w-[80px] border rounded p-2.5 flex flex-col items-center transition-colors ${trace.signals_aligned > 0 ? 'bg-orange-500/10 border-orange-500/30' : 'bg-gray-900/50 border-gray-800'}`}>
            ...
            <div className="flex-1 min-w-[80px] bg-gray-900/50 border border-gray-800 rounded p-2.5 flex flex-col items-center">
              ...
              <span className="text-[7px] text-gray-600 uppercase mt-1">min(100, raw×shock)</span>
            </div>

            <div className="text-gray-700 text-xs font-mono text-[8px]">EMA↓</div>

            {/* Box 4: Previous Score — NEW, was missing, required to verify EMA output */}
            <div className="flex-1 min-w-[80px] bg-gray-900/30 border border-gray-700/50 rounded p-2.5 flex flex-col items-center">
            ...
            {/* Box 5: Alpha */}
            <div className={`flex-1 min-w-[80px] border rounded p-2.5 flex flex-col items-center transition-colors ${trace.smoothing_alpha === 0.35 ? 'bg-red-500/10 border-red-500/30' : 'bg-emerald-500/10 border-emerald-500/30'}`}>
            ...
            {/* Box 6: Final */}
            <div className="flex-1 min-w-[80px] bg-cyan-400/10 border border-cyan-400/40 rounded p-2.5 flex flex-col items-center shadow-[0_0_15px_rgba(34,211,238,0.1)]">
              ...
              <span className="text-[7px] text-cyan-400/60 uppercase mt-1">
                ≈{verifiedSmoothed.toFixed(1)} rounded
              </span>
            </div>
          </div>

          {/* EMA formula written out explicitly — fully checkable */}
          <div className="text-[9px] font-mono text-gray-600 bg-gray-900/30 rounded px-3 py-2 border border-gray-800/50">
            EMA = {trace.smoothing_alpha}×{trace.pre_smooth_score.toFixed(1)} + {(1-trace.smoothing_alpha).toFixed(2)}×{trace.previous_score.toFixed(1)} = <span className="text-cyan-400 font-black">{verifiedSmoothed.toFixed(1)}</span> → rounded → <span className="text-white font-black">{verifiedFinal}</span>
            {' '}<span className={pipelineVerified ? 'text-emerald-500' : 'text-red-500'}>{pipelineVerified ? '✓' : '⚠'}</span>
          </div>
        </div>
        ...
        {/* ── Section 04: Computation Narrative ──────────────────────────── */}
        <div className="col-span-12 lg:col-span-6 flex flex-col gap-4">
          <h3 className="text-[8px] text-cyan-400/60 uppercase tracking-widest font-mono">04 · Computation Narrative</h3>
