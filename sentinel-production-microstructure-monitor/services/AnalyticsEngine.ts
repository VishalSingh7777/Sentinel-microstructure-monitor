...
export class AnalyticsEngine {
  ...
  // ── OFI state — holds last real value so empty windows never flicker ───────
  ...
  private processLiquidity(tick: NormalizedMarketTick): SignalOutput {
    ...
    const risk = safeNum(
      depthChange >= 0 ? 0 : Math.min(100, (-depthChange / 40) * 100),
      0
    );
  ...
  // FIX: when a 100ms window has no trades (totalVol=0), hold the last real
  // value instead of defaultSignal("Insufficient data"). This kills the flicker
  // caused by BinanceService clearing recentTrades after every tick emission.
  private processFlow(tick: NormalizedMarketTick): SignalOutput {
    ...
    if (totalVol === 0) {
      ...
      return {
        ...
        explanation: prev > 65
          ? 'Aggressive market selling detected.'
  ...
  private processVolatility(tick: NormalizedMarketTick): SignalOutput {
    ...
    const getStd = (arr: number[]) => {
      ...
      return Math.sqrt(safeNum(
        arr.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / arr.length, 0
      ));
    };
  ...
  // ── Signal 4: Forced Selling ───────────────────────────────────────────────
  // FIX 1: Denominator raised 5→10 BTC. Previously 5 BTC clamped to 100 on
  //   any moderate block. Now 10 BTC=100 giving a graduated response:
  //   2-4 BTC blocks → 20-40, 8+ BTC → 80+.
  // FIX 2: Confidence now measures large trade COUNT not total trade count.
  //   Old threshold needed 50 total trades per 100ms (impossible in practice).
  //   New: ≥3 block trades=HIGH, ≥1=MEDIUM, 0=LOW.
  private processForcedSelling(tick: NormalizedMarketTick): SignalOutput {
    ...
    const confidence = largeSells.length >= 3
    ...
    return {
      ...
      triggered:   risk > 45,
      raw_metrics: {
        'Whale Vol': totalLargeVol.toFixed(1),
        'Count':     largeSells.length,
      },
      explanation: risk > 45
        ? `Large seller active: ${totalLargeVol.toFixed(1)} BTC sold in blocks.`
  ...
  private calculateStressWithTrace(
    ...
    const confValues = sigArray.map(s =>
      s.confidence === ConfidenceLevel.HIGH ? 3 :
      s.confidence === ConfidenceLevel.MEDIUM ? 2 : 1
    );
    ...
    const weight_contributions: WeightContribution[] = sigArray.map(sig => {
      ...
      return {
        signal: sig.name, weight, raw_value: sig.value, contribution,
    ...
    const confidence_reasons: Record<string, string> = {
      ...
      // FIX: was "BTC/s" — threshold is 2.0 BTC per 100ms tick, not per second (20 BTC/s would be wrong by 10×)
      [SignalType.FLOW]:           `Volume this tick — need ≥2.0 BTC per tick for HIGH, ≥0.5 for MEDIUM`,
    ...
    // Build smoothing formula string so the final score is fully verifiable from the narrative alone
    const smoothingFormula = `${alpha} × ${targetStress.toFixed(1)} + ${(1 - alpha).toFixed(2)} × ${this.previousStress.toFixed(1)} = ${smoothedStress.toFixed(1)}`;

    // FIX: was "signal(s) triggered simultaneously" — "simultaneously" is wrong for single signals
    // and plural handling was sloppy. Now correct singular/plural with no incorrect simultaneity claim.
    const shockNote = activeSignals > 0
      ? `Shock multiplier ${shockMultiplier.toFixed(2)}× applied (${activeSignals} ${activeSignals === 1 ? 'signal' : 'signals'} active), elevating score to ${targetStress.toFixed(1)}. `
      : '';
    ...
    const audit_narrative = rawStress > 0
      ? `Dominant contributor: ${dominant.signal} (${(dominant.weight * 100).toFixed(0)}% weight × ${dominant.raw_value} raw = ${dominant.contribution.toFixed(1)} pts, ${dominant.pct_of_total.toFixed(1)}% of raw score). Raw weighted score: ${rawStress.toFixed(1)} pts. ${shockNote}Stress is ${direction}, so smoothing alpha = ${alpha} (${alpha === 0.35 ? 'fast-attack' : 'slow-decay'}). EMA: ${smoothingFormula} → rounded to ${finalScore} (${level}). System confidence: ${globalConfidence}.`
      : `All signals stable. Score: 0. System monitoring.`;

    const trace: DecisionTrace = {
      weight_contributions, raw_score: rawStress, signals_aligned: activeSignals,
      shock_multiplier: shockMultiplier, pre_smooth_score: targetStress,
      smoothing_alpha: alpha, previous_score: this.previousStress,
      final_score: finalScore, confidence_reasons, audit_narrative, timestamp: Date.now(),
    };
    ...
    const stress: StressScore = {
      score: finalScore, raw_score: rawStress, level, color: THEME.stress[level],
      signals_aligned: activeSignals, confidence: globalConfidence,
      breakdown: {
  ...
  private buildCausalSequence(
    ...
    const steps: CausalStep[] = this.triggerOrder.map((trigger, index) => ({
      sequence_id: index + 1,
      type: index === 0 ? 'CATALYST' : stress.score > 70 ? 'SYSTEMIC' : 'AMPLIFIER',
      signal:      trigger.signal,
      description: signals[trigger.signal].explanation,
      severity:    signals[trigger.signal].severity,
    ...
    return {
      active: true, steps, catalyst_id: catalyst, narrative,
      risk_assessment:
  ...
  private generateNarrative(steps: CausalStep[], stress: StressScore): string {
    ...
    return systemicCount > 0
      ? `Systemic breakdown initiated by ${catalyst}, now exacerbated by ${steps.length - 1} secondary amplifiers.`
      : `Tension detected in ${catalyst}, causing feedback in ${steps[steps.length - 1].signal}.`;
  }

  private detectCriticalEvent(
    tick: NormalizedMarketTick, stress: StressScore, causal: CausalSequence
  ): CriticalEvent | null {
