...
const App: React.FC = () => {
  ...
  const handleLiveTick = useCallback((tick: NormalizedMarketTick) => {
    ...
    if (result.criticalEvent) {
      logCriticalEvent(result.criticalEvent);
    }
  ...
  const runHistoryStep = useCallback((stepIndex: number, isSeeking = false) => {
    const point = historicalPoints[stepIndex];
    if (!point) return;

    const tick = historyLoader.convertToTick(point);
    
    if (isSeeking) {
      const historyPoints: TimelineDataPoint[] = [];
      const windowSize = 100;
      const startIdx = Math.max(0, stepIndex - windowSize);
      const tempAnalytics = new AnalyticsEngine();
      for (let i = startIdx; i <= stepIndex; i++) {
        const hPoint = historicalPoints[i];
        if (hPoint) {
    ...
    if (result.criticalEvent && !isSeeking) {
      logCriticalEvent(result.criticalEvent);
    }
  ...
  }, [historicalPoints, historyLoader, logCriticalEvent]);

  // Mode management Effect
  useEffect(() => {
  ...
  useEffect(() => {
    if (mode === 'HISTORICAL' && !isPaused && historicalPoints.length > 0) {
      simTimerRef.current = setInterval(() => {
        setSimStep(prev => {
          const next = prev + 1;
          if (next >= historicalPoints.length) {
            setIsPaused(true);
            return prev;
          }
          return next;
  ...
  }, [mode, isPaused, playbackSpeed, historicalPoints]);

  useEffect(() => {
    if (mode === 'HISTORICAL' && historicalPoints.length > 0) {
      runHistoryStep(simStep, false);
    }
    ...
    <div className="min-h-screen flex flex-col p-4 gap-4 max-w-[1920px] mx-auto overflow-hidden">
      <header className="flex items-center justify-between bg-[#151a23] border border-gray-800 rounded-xl px-6 py-4 shadow-2xl">
        <div className="flex items-center gap-6">
          ...
          <button 
            onClick={toggleAudio}
            className={`w-9 h-9 flex items-center justify-center rounded-lg border transition-all duration-300 ${isAudioEnabled ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.2)]' : 'bg-gray-800 border-gray-700 text-gray-500 hover:text-gray-300 hover:border-gray-500'}`}
            title={isAudioEnabled ? "Mute Sonification" : "Enable Sonification"}
          >
            {isAudioEnabled ? (
          ...
          {isLoadingHistory ? (
            ...
            <>
              <button 
                ...
                className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-800 hover:bg-amber-600 text-white transition-all shadow-inner"
              >
      ...
      <main className="flex-1 grid grid-cols-12 gap-4 h-[calc(100vh-200px)] overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div className="flex flex-col gap-3">
            {signals && Object.values(signals).map((s, i) => (
                ...
                <div className="flex gap-1.5">
                  {signals && [
                    ...
                    return (
                      <div 
                        key={v.id} 
                        className={`w-6 h-6 flex items-center justify-center rounded-sm text-[9px] font-black font-mono border transition-all duration-500 ${
                          active 
                          ? 'bg-red-500/20 border-red-500 text-red-400 shadow-[0_0_12px_rgba(239,68,68,0.4)] animate-pulse' 
                          : 'bg-gray-800/40 border-gray-800 text-gray-600 grayscale'
                        }`}
                        title={v.type}
                      >
                        {v.id}
                ...
                <div className="flex-1 overflow-y-auto space-y-0 relative pr-1 custom-scrollbar">
                   ...
                   {causal?.active ? causal.steps.map((step, i) => (
                     <div key={i} className="flex gap-8 items-start relative pl-10 pb-6 last:pb-2 group animate-in slide-in-from-left duration-700">
                        <div className={`absolute left-[16px] top-1.5 w-3 h-3 rounded-full border-2 bg-[#151a23] transition-all duration-300 z-10 
                          ${step.type === 'CATALYST' ? 'border-amber-500 scale-125 shadow-[0_0_10px_#f59e0b]' : 
                            step.type === 'SYSTEMIC' ? 'border-red-500 scale-110 shadow-[0_0_10px_#ef4444]' : 'border-gray-700'}`} />
                        
                        <div className="flex-1">
                           <div className="flex items-center justify-between mb-1.5">
                             <div className="flex items-center gap-2">
                               <span className={`text-[8px] font-mono px-2 py-0.5 rounded uppercase tracking-tighter
                                 ${step.type === 'CATALYST' ? 'bg-amber-500 text-black font-black shadow-[0_0_10px_rgba(245,158,11,0.4)]' : 
                                   step.type === 'SYSTEMIC' ? 'bg-red-500 text-white font-black' : 'bg-gray-800 text-gray-500 font-bold'}`}>
                                 {step.type}
                               </span>
                             ...
                             <span className="text-[9px] text-gray-700 font-mono italic">
                               T+{Math.round((Date.now() - step.timestamp)/1000)}s
                             </span>
                           </div>
                           <p className="text-[11px] text-gray-400 leading-snug font-sans bg-gray-900/50 p-3 rounded-lg border border-gray-800/80 hover:border-gray-700 transition-colors shadow-inner">
                             {step.description}
                           </p>
                        </div>
                     </div>
                     ...
                     <div className="h-full flex flex-col items-center justify-center text-gray-700 space-y-5 opacity-20 grayscale">
                        ...
                        <span className="text-[9px] font-mono italic uppercase tracking-widest text-center leading-loose">
                          Awaiting structural anomalies...<br/>Continuous state monitoring active.
                        </span>
                     </div>
                   )}
                </div>
             ...
             <div className="col-span-12 lg:col-span-7 bg-[#151a23] border border-gray-800 rounded-xl p-5 flex flex-col overflow-hidden shadow-2xl relative">
                ...
                <div className="flex justify-between items-center mb-5 relative z-10">
                  ...
                  <div className="flex gap-2">
                    <span className="text-[9px] text-gray-600 font-mono px-3 py-1 bg-[#0a0e14] border border-gray-800 rounded-full flex items-center gap-2 shadow-inner">
                      ...
                      {criticalLog.length} INCIDENTS
                    </span>
                    <button 
                      type="button"
                      onClick={(e) => clearAllIncidents(e)}
                      className="text-[9px] text-gray-200 font-black font-mono px-3 py-1 bg-gray-800 hover:bg-gray-700 active:bg-gray-600 rounded border border-gray-700 transition-all uppercase tracking-widest shadow-lg cursor-pointer hover:border-gray-400"
                    >
                      Flush
                    </button>
                  </div>
                   ...
                   {criticalLog.length > 0 ? criticalLog.map((event) => (
                     <div key={event.id} className="p-4 bg-[#0a0e14]/40 backdrop-blur-md border border-gray-800/60 rounded-xl hover:border-gray-600 transition-all group relative overflow-hidden shadow-sm">
                        <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${
                          event.stress_score > 80 ? 'bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.6)]' : 'bg-amber-500'
                        }`} />
                        
                        <div className="flex justify-between items-start mb-2.5">
                           <div className="flex flex-col">
                             <span className={`text-[10px] font-black font-mono uppercase tracking-tight ${event.stress_score > 80 ? 'text-red-400' : 'text-gray-300'}`}>
                               {event.level} REGIME
                             </span>
                             <span className="text-[9px] text-gray-600 font-mono">
                               {new Date(event.timestamp).toLocaleTimeString([], { hour12: false, minute: '2-digit', second: '2-digit' })} • REF_{event.id.split('_').pop()?.toUpperCase()}
                             </span>
                           </div>
                           <div className="flex items-center gap-3">
                             <div className="text-right">
                               <div className="text-[13px] font-black text-white font-mono tracking-tighter">
                                 ${event.price.toLocaleString(undefined, { minimumFractionDigits: 1 })}
                               </div>
                               <div className={`text-[9px] font-black font-mono ${event.stress_score > 70 ? 'text-red-500' : 'text-amber-500'}`}>
                                 STRESS: {event.stress_score}
                               </div>
                             </div>
                             <button 
                               type="button"
                               onClick={() => removeIncident(event.id)}
                               className="opacity-0 group-hover:opacity-100 p-1.5 bg-gray-800 hover:bg-red-900/40 text-gray-500 hover:text-red-500 rounded border border-gray-700 hover:border-red-900/60 transition-all"
                        ...
                        <div className="text-[11px] text-gray-400 leading-snug mb-3 font-sans opacity-90 group-hover:opacity-100 border-l border-gray-800/80 pl-4 py-1 italic bg-gray-900/20 rounded-r">
                           {event.narrative}
                        </div>
                        ...
                        <div className="flex gap-2 flex-wrap items-center">
                           ...
                           {event.signals.map((sig, idx) => (
                             <span key={idx} className="text-[8px] bg-[#0a0e14] text-blue-400 px-2 py-0.5 rounded-sm border border-blue-500/10 font-mono font-black hover:border-blue-400/50 transition-colors shadow-inner">
                               {sig.split(' ')[0]}
                             </span>
                           ))}
                        </div>
                     ...
                     <div className="h-full flex flex-col items-center justify-center text-gray-800 space-y-3 opacity-30">
                        ...
                        <span className="text-[9px] font-mono italic uppercase tracking-[0.4em] text-center">
                ...
                <div className="mt-4 pt-4 border-t border-gray-800/40 flex justify-between items-center bg-[#151a23]/60 relative z-10">
                   <p className="text-[8px] text-gray-600 leading-tight max-w-[75%] font-mono uppercase tracking-widest opacity-80">
                     STATE CAPTURE: ADAPTIVE NEURAL SAMPLING AT S-RANK UPGRADE.
                   </p>
